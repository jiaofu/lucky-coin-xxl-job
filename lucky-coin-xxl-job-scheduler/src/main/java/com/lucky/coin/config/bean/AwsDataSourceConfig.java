...
public class AwsDataSourceConfig {
    ...
    public String poolJdbcUrl() {
        return String.format("jdbc:%s://%s:%s/pool?autoReconnect=true&useUnicode=true&characterEncoding=utf-8", engine, host, port);
    }

    public String btcJdbcUrl() {
        return String.format("jdbc:%s://%s:%s/btc?autoReconnect=true&useUnicode=true&characterEncoding=utf-8", engine, host, port);
    }
